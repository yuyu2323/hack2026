...
def start(concepts, without_worker):
    ...
    for name, port in [('ai', 8201), ('api', 8101)] + [(f'concept-{number:02d}', 5181) for number in concepts]:
        require_available_port(name, port)
    launch('ai',[str(ROOT/'ai-service/.venv/bin/python'),'-m','uvicorn','app.main:app','--app-dir',str(ROOT/'ai-service'),'--host','127.0.0.1','--port','8201','--no-access-log'],'app.main:app',8201)
    launch('api',[str(ROOT/'server/.venv/bin/python'),'-m','uvicorn','server.main:app','--app-dir',str(ROOT),'--host','127.0.0.1','--port','8101','--no-access-log'],'server.main:app',8101)
    for port in (8201, 8101):
    ...
    for number in concepts:
        ...
        launch(f'concept-{number:02d}',[shutil.which('node'),str(ROOT/'node_modules/vite/bin/vite.js'),'--host','127.0.0.1','--port',str(5181),'--strictPort'],f'--port {5181}',port=5181,cwd=package)
        print(f'시안 {number:02d}: http://127.0.0.1:{5181}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--concept', type=int, choices=[1], action='append')
