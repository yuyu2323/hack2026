...
def start(concepts, without_worker):
    ...
    for name, port in [('ai', 8202), ('api', 8102)] + [(f'concept-{number:02d}', 5182) for number in concepts]:
        require_available_port(name, port)
    launch('ai',[str(ROOT/'ai-service/.venv/bin/python'),'-m','uvicorn','app.main:app','--app-dir',str(ROOT/'ai-service'),'--host','127.0.0.1','--port','8202','--no-access-log'],'app.main:app',8202)
    launch('api',[str(ROOT/'server/.venv/bin/python'),'-m','uvicorn','server.main:app','--app-dir',str(ROOT),'--host','127.0.0.1','--port','8102','--no-access-log'],'server.main:app',8102)
    for port in (8202, 8102):
    ...
    for number in concepts:
        ...
        launch(f'concept-{number:02d}',[shutil.which('node'),str(ROOT/'node_modules/vite/bin/vite.js'),'--host','127.0.0.1','--port',str(5182),'--strictPort'],f'--port {5182}',port=5182,cwd=package)
        print(f'시안 {number:02d}: http://127.0.0.1:{5182}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--concept', type=int, choices=[2], action='append')
    parser.add_argument('--without-worker', action='store_true')
    args = parser.parse_args()
    start(args.concept or [2], args.without_worker) if args.action == 'start' else stop()
