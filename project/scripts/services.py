...
def start(concepts, without_worker):
    ...
    for name, port in [('ai', 8204), ('api', 8104)] + [(f'concept-{number:02d}', 5184) for number in concepts]:
        require_available_port(name, port)
    launch('ai',[str(ROOT/'ai-service/.venv/bin/python'),'-m','uvicorn','app.main:app','--app-dir',str(ROOT/'ai-service'),'--host','127.0.0.1','--port','8204','--no-access-log'],'app.main:app',8204)
    launch('api',[str(ROOT/'server/.venv/bin/python'),'-m','uvicorn','server.main:app','--app-dir',str(ROOT),'--host','127.0.0.1','--port','8104','--no-access-log'],'server.main:app',8104)
    for port in (8204, 8104):
    ...
    for number in concepts:
        ...
        launch(f'concept-{number:02d}',[shutil.which('node'),str(ROOT/'node_modules/vite/bin/vite.js'),'--host','127.0.0.1','--port',str(5184),'--strictPort'],f'--port {5184}',port=5184,cwd=package)
        print(f'시안 {number:02d}: http://127.0.0.1:{5184}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--concept', type=int, choices=[4], action='append')
    parser.add_argument('--without-worker', action='store_true')
    args = parser.parse_args()
    start(args.concept or [4], args.without_worker) if args.action == 'start' else stop()
