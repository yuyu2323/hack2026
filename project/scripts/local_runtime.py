...
def prepare():
    ...
    if not runtime_file.exists():
        origins = [f'http://{host}:{port}' for host in ['127.0.0.1', 'localhost'] for port in [5182]]
        values = {
            'DATABASE_URL': f'postgresql+psycopg://storeloop:{password}@127.0.0.1:55442/storeloop',
            'STORELOOP_TEST_DATABASE_URL': f'postgresql+psycopg://storeloop:{password}@127.0.0.1:55442/storeloop_test',
            'MEDIA_ROOT': str(LOCAL / 'media'),
            'AI_SERVICE_URL': 'http://127.0.0.1:8202',
            'AI_SERVICE_TOKEN': secrets.token_urlsafe(48),
            'SESSION_COOKIE_SECURE': 'false',
            'SESSION_COOKIE_NAME': 'storeloop_concept_02_session',
            'ALLOWED_ORIGINS': json.dumps(origins),
            'AI_PROVIDER': os.environ.get('AI_PROVIDER', 'codex'),
            'OPENAI_MODEL': os.environ.get('OPENAI_MODEL', 'gpt-4.1-mini'),
            'OPENAI_MAX_OUTPUT_TOKENS': '6000',
            'AI_REQUESTS_ENABLED': os.environ.get('AI_REQUESTS_ENABLED', 'true'),
...
def database(action):
    ...
    if status.returncode:
        ...
        subprocess.run([ctl, '-D', str(data), '-l', str(LOCAL / 'logs/postgres.log'),
                        '-o', '-h 127.0.0.1 -p 55442 -k /tmp', '-w', 'start'], check=True)
    env = dict(os.environ, PGPASSWORD=(LOCAL / 'postgres-password').read_text().strip())
    command = [pg_binary('psql'), '-h', '127.0.0.1', '-p', '55442', '-U', 'storeloop', '-d', 'postgres', '-At']
    ...
    print('전용 PostgreSQL 준비 완료: 127.0.0.1:55442 / storeloop, storeloop_test')
